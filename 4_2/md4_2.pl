% salîdzinâðanas predikâti, tiek izmantoti, jo termini ir salikti no 3 daþâdâm vçrtîbâm
...
% koka apstaigâðanas funkcijas, ja virsotne ir lapa vai arî ar vienu bçrnu tad rezultâtam nonâkot tajâ, tas ir jau sakârtots
aa(n0(A), X) 		:- min_v(A, Min), max_v(A, Max), length(A, Len), X = [(Len, Min, Max)]. % atrod minâlo, maksimâlo vçrtîbu un saraksta garumumu
aa(n1(T1), X) 		:- aa(T1, Y), append(Y, [], X).
aa(n2(T1, T2), X) 	:- aa(T1, Y), aa(T2, Z), merge(Y, Z, X). % tâ kâ ðajâ gadîjumâ ir divi rezultâti tad tie ir jâsavieno kopâ, lai rezultâts bûtu sakârtots

% veic katra saraksta elementa kâpinâðanu kvadrâtâ, darbs notiek rekursîvi
double_list([], X)	:- X = [].
double_list([Xh | Xt], X)	:- Y is Xh * Xh, double_list(Xt, Z), append([Y], Z, X).

% veic koka apstaigâðanu un rekonstruçðanu
